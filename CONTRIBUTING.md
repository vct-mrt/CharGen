...
Thank you for your interest in contributing to CharGen, a small CLI tool that generates random strings from selectable character sets. Contributions of all kinds are welcome: bug reports, fixes, tests, and documentation improvements.
...
- Functions return `84` on error, `0` on success. This value propagates as the process exit code.
- Hand-rolled libc functions (`my_strlen`, `str_compare`, `my_getnbr`, etc.) are kept deliberately; do not replace them with stdlib equivalents.
- Indentation is 4 spaces, no tabs, in C/H files. See `.editorconfig`.
- Code must compile without warnings under `-W -Wall -Wextra -O2`. CI also builds with `-Werror`.
- `cppcheck --enable=all` must report zero findings. CI blocks the pipeline on any finding.
- CI builds with `-fsanitize=address,undefined`. New code must be clean under ASan/UBSan.
- CharGen uses `rand()` and is not cryptographically secure. Do not add features that imply security guarantees, and do not remove the warnings in README and SECURITY.md.
